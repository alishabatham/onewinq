    ...
    <div className="bg-slate-50 text-slate-800 min-h-screen flex">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 shrink-0 h-screen sticky top-0 overflow-hidden">
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center space-x-3 px-4 py-3 mb-2 bg-slate-50 rounded-xl border border-slate-100">
            <div className="bg-brand text-white h-9 w-9 rounded-full flex items-center justify-center font-bold text-sm shrink-0 border border-brand/20 shadow-xs">
              {user?.name?.charAt(0).toUpperCase()}
            </div>
            <div className="overflow-hidden text-left">
              <p className="text-sm font-bold truncate text-slate-800">{user?.name}</p>
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider mt-0.5 ${
                user?.subscriptionStatus === 'pro' || user?.plan === 'pro'
                  ? 'bg-gradient-to-r from-amber-500 to-indigo-600 text-white shadow-xs'
                  : 'bg-slate-200 text-slate-700 border border-slate-300'
              }`}>
                {user?.subscriptionStatus === 'pro' || user?.plan === 'pro' ? '★ Pro Plan' : 'Free Plan'}
              </span>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="flex items-center space-x-3 w-full px-4 py-2 rounded-lg text-xs font-bold text-red-600 hover:bg-red-50 transition-all text-left cursor-pointer"
          >
        ...
        <div className="fixed inset-0 z-40 md:hidden flex">
          ...
          <aside className="relative flex flex-col w-64 max-w-xs bg-white border-r border-slate-200 z-50 h-full p-4">
            ...
            <div className="p-2 border-t border-slate-100 mt-auto">
              <div className="flex items-center space-x-3 px-3 py-2 mb-2 bg-slate-50 rounded-xl border border-slate-100">
                <div className="bg-brand text-white h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0">
                  {user?.name?.charAt(0).toUpperCase()}
                </div>
                <div className="overflow-hidden text-left">
